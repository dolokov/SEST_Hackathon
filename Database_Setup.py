...
for i in range(NUMBER_OF_JOBS):
	...
	time = datetime.now() - timedelta(seconds=random()*60*60*4)
	latest_arrival = time + timedelta(seconds = random() * 60*60*24)
	bikeID = conn.execute('SELECT ID FROM bikes WHERE active_job = 0 ORDER BY RANDOM() LIMIT 1').fetchone()[0]

	conn.execute('INSERT INTO jobs VALUES(?,?,?,?,?,?)', (i, target_lat, target_lon, time, latest_arrival, bikeID))
